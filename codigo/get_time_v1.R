...
#   #date_pattern_es <- "\\b(?:\\d{1,2}[\\/\\-]\\d{1,2}[\\/\\-](?:\\d{2})?\\d{2}|\\d{1,2}\\s+de\\s+(?:enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|setiembre|octubre|noviembre|diciembre)\\s+de\\s+(?:\\d{2})?\\d{2})\\b"
...
#   date_pattern_en <- "\\b(?:\\d{1,2}[\\/\\-]\\d{1,2}[\\/\\-](?:\\d{2})?\\d{2}|\\d{1,2}\\s+(?:January|February|March|April|May|June|July|August|September|October|November|December)\\s+\\d{4})\\b"
...
#   for (line in lines) {
#     date_in_line_es <- str_extract(line, date_pattern_es)
#     date_in_line_en <- str_extract(line, date_pattern_en)
    
#     if (!is.na(date_in_line_es)) {
#       if (grepl("\\d{1,2}[\\/\\-]\\d{1,2}[\\/\\-](?:\\d{2})?\\d{2}", date_in_line_es)) {
